    ...
    this.api.login(emailInput.value, passwordInput.value).subscribe(data => {
      ...
      if (data.error == undefined) {
        if (data.result.role === "administrator") {
          ...
          this.globals.user = { name: data.result.first_name + " " + data.result.last_name, email: data.result.email, role: data.result.role, username: data.result.username, id: data.result.id }
